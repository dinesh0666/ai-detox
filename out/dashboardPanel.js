...
class DashboardPanel {
    static createOrShow(context, session, stats) {
        if (DashboardPanel.currentPanel) {
            DashboardPanel.currentPanel.panel.reveal();
            DashboardPanel.currentPanel.update(session, stats);
            return;
        }
        const panel = vscode.window.createWebviewPanel('aiDetoxDashboard', 'AI Detox Dashboard', vscode.ViewColumn.One, { enableScripts: true });
        DashboardPanel.currentPanel = new DashboardPanel(panel, context, session, stats);
    }
    constructor(panel, context, session, stats) {
